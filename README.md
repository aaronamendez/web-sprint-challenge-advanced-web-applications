...
A token is a hashed value that represents encrypted data for a set of properties and values. We can use this token to authenticate a user by passing it in our requests/headers whenever we make a request that requires that level of authorization for any data we wish to achieve.

2. What steps can you take in your web apps to keep your data secure?

We can use proper validations so that we receive data only when it is in a expected format, and then add in middleware that checks/verify's that the data is to be expected and properly read both on the client side and server side.

3. Describe how web servers work.
   It is a series of tubes! (Joking!) The web servers exist on a machine which can be virtual/physical, and we can then use the web server to make or receive requests and pass any data along with/for that request.

4. Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

Post for the C/Create
Get for the R/Read
Put for the U/Update
Delete for the D/Delete

Warm Thanks for reviewing my answers/code,
Have a great day!
